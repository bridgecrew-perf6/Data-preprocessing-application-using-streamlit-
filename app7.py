...
we need to  preprocess the data before building any model / Ml model
preprocess meaning there should not be any missing values , outliers in the dataset
In the best scenario the data should be feature scaled means need to do feature engineering
before building any ml model
The main aim behind building this app is any user can upload the dataset in csv or xlsx format
and preprocess the data with clicks of button """
...
st.title("Data preprocessing application using streamlit")
st.markdown("This project is done by Sowjanya")
...
def mvt_mean(df):
    try:
        ...
        st.info("The Percenatage of Value Missing in Given Data is : {:.2f}%".format(
...
def mvt_median(df):
    try:
        ...
        st.dataframe(clean_df)
        st.write(df.dtypes)
        st.info("The Percenatge of Value Missing in Given Data is : {:.2f}%".format(
            ((df.isna().sum().sum()) / (df.count().sum()) * 100)))
...
def mvt_mode(df):
    try:
        ...
        st.info("The Percentage of Value Missing in Given Data is : {:.3f}%".format(
            (df[cat_col].isna().sum().sum()) / (df.count().sum()) * 100))
        st.info("\nThe Percenatge of Value Missing in Given Data is :\n{}".format(
            (df[cat_col].isnull().sum() * 100) / df.shape[0]))
...
def mail():
    try:
        ...
        '''
        # os.chdir(path) mail address & password
        file_name = 'pass.txt'
        if os.path.exists(file_name):
            ...
            urllib.request.urlretrieve(
                "https://drive.google.com/file/d/1tan_wJsUqOtBTJv1lrwpqqJYgdVJY1td/view?usp=sharing", "pass.txt")
            with open('pass.txt', 'r') as file:
                sender_pass = file.read()
                file.close()

        sender_address = 'datapreproces@gmail.com'
        ...
        if receiver_address:
            if (re.search(regex, receiver_address)):
                ...
                # The subject line , body and the attachments for the mail
